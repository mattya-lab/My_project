...
class IELTS_Vocabrary:
    ...
    def IndexGetWord(self, num, level):
    ...
    def BookOrderWord(self, nowlevel, num):
        if nowlevel == 0:
            self.number = self.level1[num][0]
            self.word = self.level1[num][1]
            self.sentence = self.level1[num][2]
        ...
        return self.number, self.word, self.sentence, nowlevel, num 
    ...
    def Graphic_Drawing(self):
        ...
        while (True):
            ...
            try:
                ...
                #self.number, self.word, self.semtence = self.IndexGetWord(index, level)
                #if index == 500:
                    #level = level + 1
                    #if index == 500 and level == (3+1):
                        #level == 0
                    #index = 0
